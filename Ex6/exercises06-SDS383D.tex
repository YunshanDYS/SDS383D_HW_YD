...
\begin{align*}
    ...
    &= \frac{1}{s_2 - s_1^2} 
    \begin{bmatrix}
    s_2 - (x_1 -x)s_1 & \dots & s_2 - (x_n -x)s_1 \\
    -s_1 + (x_1 - x) & \dots & - s_1 + (x_n - x) 
    \end{bmatrix}
    \begin{bmatrix}
    w_1 y_1 \\
    \vdots \\ 
    w_n y_n
    \end{bmatrix} \\
    \hat{f}(x) &= \frac{1}{s_2 - s_1^2} \sum_i [s_2 - (x_i - x)s_1] K(\frac{x_i - x}{h})y_i \\
    &= \frac{1}{s_2 - s_1^2} \sum_i w_i (x) y_i
\end{align*}
And,
\begin{align*}
    \sum_i w_i (x) &= \sum_i K(\frac{x_i - x}{h}) \{s_2(x) - (x_i - x) s_1(x)\} \\
    &= s_2(x) - s_1^2(x)
\end{align*}
Therefore,
\begin{align*}
    \hat{f}(x) = \frac{\sum_i w_i(x) y_i}{\sum_i w_i(x)}.
\end{align*}
\bigskip
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.6\textwidth}
        \centering
        \includegraphics[width=\linewidth]{Ex6/figures/GP_tau2sq_seq_M52.png} 
        %\caption{Generic} \label{fig:timing1}
    \end{subfigure}
    \caption{Simulated Gaussian process with various hyper parameters using the Matern covariance function.}
    \label{fig:GPM52}
\end{figure}
...
\bigskip \jie
For a new point $x^*$, we can write the joint distribution of $f(x)$ and $f(x^*)$ as follows,
\begin{align*}
        \begin{bmatrix}
f(x^*) \\
f(x)
...
\end{bmatrix}\right),
\end{align*}
where $C^* = C(x^*, x^*)$, $\tilde{C} = C(x, x^*)$, $C = C(x, x)$.
