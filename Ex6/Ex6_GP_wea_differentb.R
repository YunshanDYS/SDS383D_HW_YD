...
b2_seq <- seq(0.01,5,length.out = 20)
grid <- expand.grid(tau1sq_seq,b1_seq,b2_seq)
lp <- rep(NA,nrow(grid))
for (i in 1:nrow(grid)){
  lp[i] <- logmar(y, X, b = c(grid[i,2], grid[i,3]), tau1sq = grid[i,1], tau2sq = 1e-6)
  if (i %%100 == 0){cat(i,"\t")}
}

# plot heatmap
df <- data.frame(tau1sq = grid[,1], b1 = grid[,2], b2 = grid[,3], lp = lp)
...
png("figures/pressure2.png",width=800,height=600)
# png("figures/temp2.png",width=800,height=600)
par(mfrow = c(1, 2))
